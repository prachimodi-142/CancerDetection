...
BASE_DIR = os.path.join(".", "test_patient_data")
...
def get_training_slide_path(slide_number):
  ...
  slide_filepath = os.path.join(BASE_DIR, TRAIN_PREFIX + padded_sl_num + "." + SRC_TRAIN_EXT)
...
def slide_to_scaled_pil_image(slide_number):
  ...
  whole_slide_image_rgb = whole_slide_image.convert('RGB')
  img_rgb = whole_slide_image_rgb.resize((new_w, new_h), PIL.Image.BILINEAR)
  return img_rgb, large_w, large_h, new_w, new_h
...
def get_num_training_slides():
  ...
  """
  num_training_slides = len(glob.glob1(BASE_DIR, "*." + SRC_TRAIN_EXT))
